...
class ConvertionJob implements ShouldQueue
{
    ...
    private function split(): void
    {
        try {
            ...
            $method = $this->splitOptions['method'] ?? 'sheets';
            $format = $this->splitOptions['format'] ?? 'zip';
            $rowsPerFile = $this->splitOptions['rows_per_file'] ?? 100;

            Log::info('Starting split operation', [
                'method' => $method,
                'format' => $format,
            ...
            if (!Storage::disk('local')->exists($this->path)) {
                throw new \Exception("Source file not found: " . $this->path);
            }

            $fileContent = Storage::disk('local')->get($this->path);
            $tempFilePath = tempnam(sys_get_temp_dir(), 'split_source_');
            file_put_contents($tempFilePath, $fileContent);

            $fileExtension = strtolower(pathinfo($this->path, PATHINFO_EXTENSION));
            
            // Определяем reader в зависимости от расширения файла
            ...
            if ($method === 'sheets') {
                $this->splitBySheets($sourceSpreadsheet, $format, $fileExtension);
            } else {
                $this->splitByRows($sourceSpreadsheet, $rowsPerFile, $format, $fileExtension);
            }
            ...
            $this->fileMetaData->update(["status" => "failed"]);
            Log::error('Function split: Error: ' . $e->getMessage());
            throw $e;
        } finally {
            // Удаляем временные файлы
    ...
    private function copyCellsWithArrayStyles($sourceSheet, $newSheet): void
    {
        try {
            ...
            Log::info("Copying cells", [
                'sheet' => $sourceSheet->getTitle(),
                'rows' => $highestRow,
                'columns' => $highestColumn
            ]);

            // Простая логика копирования для тестирования
            for ($row = 1; $row <= $highestRow; $row++) {
                for ($col = 'A'; $col <= $highestColumn; $col++) {
                    $cellCoordinate = $col . $row;
                    $cellValue = $sourceSheet->getCell($cellCoordinate)->getValue();
                    
                    if ($cellValue !== null) {
                        $newSheet->setCellValue($cellCoordinate, $cellValue);
                        
                        // Копируем базовые стили
                        try {
                            $sourceStyle = $sourceSheet->getStyle($cellCoordinate);
                            $newSheet->getStyle($cellCoordinate)->applyFromArray($sourceStyle->exportArray());
                        } catch (\Exception $e) {
                            // Игнорируем ошибки стилей для тестирования
                        }
                    }
                }
                
                // Освобождаем память каждые 50 строк
                if ($row % 50 === 0) {
                    gc_collect_cycles();
                }
            }

            Log::info("Cells copied successfully", [
                'sheet' => $sourceSheet->getTitle(),
                'total_rows' => $highestRow
            ]);

        } catch (\Exception $e) {
            Log::error('Error in copyCellsWithArrayStyles: ' . $e->getMessage());
            throw $e;
        }
    }
